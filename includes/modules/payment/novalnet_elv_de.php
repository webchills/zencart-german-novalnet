...
class novalnet_elv_de {
  ...
  function novalnet_elv_de() {
	...
	$this->vendor_id  = trim(MODULE_PAYMENT_NOVALNET_ELV_DE_VENDOR_ID);
	$this->auth_code  = trim(MODULE_PAYMENT_NOVALNET_ELV_DE_AUTH_CODE);
    $this->product_id = trim(MODULE_PAYMENT_NOVALNET_ELV_DE_PRODUCT_ID);
    $this->tariff_id = trim(MODULE_PAYMENT_NOVALNET_ELV_DE_TARIFF_ID);
	$this->manual_check_limit = trim(MODULE_PAYMENT_NOVALNET_ELV_DE_MANUAL_CHECK_LIMIT);
	$this->product_id2 = trim(MODULE_PAYMENT_NOVALNET_ELV_DE_PRODUCT_ID2);
    $this->tariff_id2  = trim(MODULE_PAYMENT_NOVALNET_ELV_DE_TARIFF_ID2);
	$this->test_mode = (strtolower(MODULE_PAYMENT_NOVALNET_ELV_DE_TEST_MODE) == 'true' or MODULE_PAYMENT_NOVALNET_ELV_DE_TEST_MODE == '1')? 1: 0;
   ...
   /*if(MODULE_PAYMENT_ELV_DE_LOGO_STATUS == 'True'){
	$this->public_title = '<NOBR> Novalnet </NOBR>'.MODULE_PAYMENT_NOVALNET_ELV_DE_TEXT_PUBLIC_TITLE;
	}*/
	
	if(MODULE_PAYMENT_ELV_DE_LOGO_STATUS == 'True'){
		$this->public_title = 'Novalnet'.' '.MODULE_PAYMENT_NOVALNET_ELV_DE_TEXT_PUBLIC_TITLE;
	...
	if($_SESSION['nn_tid_elv_de']){
	  ...
	  if( (empty($_SESSION['invalid_count_de'])) || ( isset($_SESSION['max_time_elv_de']) && (time() >= $_SESSION['max_time_elv_de'])) ){
				$_SESSION['invalid_count_de'] = 0;
			}
			 if( !empty($_SESSION['invalid_count_de']) && $_SESSION['invalid_count_de'] == 3 ){	
		
	  if($_SESSION['max_time_elv_de'] && (time() < $_SESSION['max_time_elv_de'])){
		    $payment_error_return = 'payment_error=' . $this->code . '&error=' . utf8_encode( MODULE_PAYMENT_NOVALNET_ELV_DE_PIN_BY_CALLBACK_SESSION_ERROR );
			}
	...
	//Check the time limit
		if($_SESSION['max_time_elv_de'] && time() > $_SESSION['max_time_elv_de']){
			...
			unset($_SESSION['invalid_count_de']);
	...
	if(MODULE_PAYMENT_NOVALNET_ELV_DE_PIN_BY_CALLBACK_SMS != 'False'){
   	...
   	function checkConfigure() {
		if (IS_ADMIN_FLAG == true) {
			$this->title = MODULE_PAYMENT_NOVALNET_ELV_DE_TEXT_TITLE; // Payment module title in Admin
			if(MODULE_PAYMENT_ELV_DE_LOGO_STATUS == 'True'){
				$this->public_title = 'Novalnet'.' '.MODULE_PAYMENT_NOVALNET_ELV_DE_TEXT_PUBLIC_TITLE;
				$this->title = 'Novalnet'.' '.MODULE_PAYMENT_NOVALNET_ELV_DE_TEXT_TITLE;
			}
			if ($this->enabled == 'true' && (!$this->vendor_id || !$this->auth_code || !$this->product_id || !$this->tariff_id )) {
				$this->title .=  '<span class="alert">'.MODULE_PAYMENT_NOVALNET_ELV_DE_NOT_CONFIGURED.'</span>';
			} elseif ($this->test_mode == '1') {
				$this->title .= '<span class="alert">'.MODULE_PAYMENT_NOVALNET_ELV_DE_IN_TEST_MODE.'</span>';
			}
  ...
  function selection() {
   ...
   // $onFocus = '';
		...
		$selection = array('id' => $this->code,
						   ...
						   'fields' => array(array('title' => MODULE_PAYMENT_NOVALNET_ELV_DE_TEXT_BANK_ACCOUNT_OWNER,
											 ...
											 array('title' => MODULE_PAYMENT_NOVALNET_ELV_DE_TEXT_BANK_ACCOUNT_NUMBER,
												   'field' => zen_draw_input_field('bank_account', '', 'id="' . $this->code . '-bank_account" AUTOCOMPLETE="OFF"' . $onFocus),
												   'tag' => $this->code . '-bank_account'),
											 array('title' => MODULE_PAYMENT_NOVALNET_ELV_DE_TEXT_BANK_CODE,
									'field' => zen_draw_input_field('bank_code', '', 'id="' . $this->code . '-bank_code" AUTOCOMPLETE="OFF"' . $onFocus),
							'tag' => $this->code . '-bank_code'),
							array('title' => '', 'field' => MODULE_PAYMENT_NOVALNET_INFORMATION_PAYMENT_DE),	
    ...
    // Display callback fields
		...
		if($this->isActivatedCallback && in_array($billing_iso_code, $this->nnelvde_allowed_pin_country_list) && $amount_check >= MODULE_PAYMENT_NOVALNET_ELV_DE_PIN_BY_CALLBACK_MIN_LIMIT )	
		{				   
					if( MODULE_PAYMENT_NOVALNET_ELV_DE_PIN_BY_CALLBACK_SMS == 'Email Reply' ){
					$_SESSION['user_email_elv_de'] = ($_SESSION['user_email_elv_de'] == '')?$order->customer['email_address']:$_SESSION['user_email_elv_de'];
					
					$selection['fields'][] = array( 'title' => MODULE_PAYMENT_NOVALNET_ELV_DE_PIN_BY_CALLBACK_EMAIL_REQ, 'field' => zen_draw_input_field ( 'user_email_elv_de', $_SESSION['user_email_elv_de'], 'id="'.$this->code.'-callback" AUTOCOMPLETE="OFF"'.$onFocus ) );
				}else{
					$_SESSION['user_tel_elv_de'] = ($_SESSION['user_tel_elv_de'] == '')?$order->customer['telephone']:$_SESSION['user_tel_elv_de'];
					
					$label_str =(MODULE_PAYMENT_NOVALNET_ELV_DE_PIN_BY_CALLBACK_SMS == 'Callback (Telefon & Handy)')?MODULE_PAYMENT_NOVALNET_ELV_DE_PIN_BY_CALLBACK_TEL_REQ:MODULE_PAYMENT_NOVALNET_ELV_DE_PIN_BY_CALLBACK_SMS_REQ;
					
					$selection['fields'][] = array( 'title' => $label_str, 'field' => zen_draw_input_field ( 'user_tel_elv_de', $_SESSION['user_tel_elv_de'], 'id="'.$this->code.'-callback" AUTOCOMPLETE="OFF"'.$onFocus ) );
				}
	    }
    }
	$amount_check = $_SESSION['nn_amount_elv_de'];
	
	if($this->isActivatedCallback && in_array($billing_iso_code, $this->nnelvde_allowed_pin_country_list) && $amount_check >= MODULE_PAYMENT_NOVALNET_ELV_DE_PIN_BY_CALLBACK_MIN_LIMIT && isset($_SESSION['nn_tid_elv_de']) && ($_SESSION['invalid_count_de'] < 3) )
	{
			...
			if( MODULE_PAYMENT_NOVALNET_ELV_DE_PIN_BY_CALLBACK_SMS == 'Email Reply' ){ 	
				...
				$selection['fields'][] = array( 'title' => MODULE_PAYMENT_NOVALNET_ELV_DE_PIN_BY_CALLBACK_SMS_PIN, 'field' => zen_draw_input_field( 'novalnet_pin_elv_de', '', 'id="'.$this->code.'-callback" AUTOCOMPLETE="OFF"'.$onFocus) );
  ...
  function pre_confirmation_check() {
    ...
    // print_r($_SESSION['elv_de_order_amount']); exit;  
    ...
    if($_SESSION['nn_tid_elv_de'])
    {
		...
		if($_SESSION['elv_de_order_amount'] != $amount){
		  ...
		  if(  MODULE_PAYMENT_NOVALNET_ELV_DE_PIN_BY_CALLBACK_SMS == 'Email Reply' ){ 
			$error_message = MODULE_PAYMENT_NOVALNET_ELV_DE_AMOUNT_VARIATION_MESSAGE_EMAIL;	
		}elseif( MODULE_PAYMENT_NOVALNET_ELV_DE_PIN_BY_CALLBACK_SMS == 'Callback (Telefon & Handy)'  || MODULE_PAYMENT_NOVALNET_ELV_DE_PIN_BY_CALLBACK_SMS == 'SMS (nur Handy)' )
		{
			$error_message = MODULE_PAYMENT_NOVALNET_ELV_DE_AMOUNT_VARIATION_MESSAGE;
		}
		
			unset($_SESSION['nn_tid_elv_de']);
			unset($_SESSION['elv_de_order_amount']);
			    if(isset($_SESSION['invalid_count_de']))
			{
				unset($_SESSION['invalid_count_de']);
			}	
			$payment_error_return = 'payment_error=' . $this->code;
		...
		if($HTTP_POST_VARS['novalnet_pin_elv_de'] == '' || (preg_match('/[&_#%\^<>@$=*!]/',$HTTP_POST_VARS['novalnet_pin_elv_de'])))
		{	
		$payment_error_return = 'payment_error=' . $this->code . '&error=' . utf8_encode( MODULE_PAYMENT_NOVALNET_ELV_DE_PIN_BY_CALLBACK_SMS_PIN_NOTVALID );
		zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, $payment_error_return, 'SSL', true, false));
		}else{
		...
		$error = '';
				...
				if (!function_exists('curl_init') && ($this->_code=='novalnet_elv_de')){
				ini_set('display_errors', 1);
				ini_set('error_reporting', E_ALL);

				$error =  MODULE_PAYMENT_NOVALNET_ELV_DE_CURL_MESSAGE;
				}
				...
				elseif(!$HTTP_POST_VARS['bank_account_holder'] || (preg_match('/[#%\^<>@$=*!]/',$HTTP_POST_VARS['bank_account_holder']))){
				 $error = MODULE_PAYMENT_NOVALNET_ELV_DE_TEXT_JS_BANK_ACCOUNT_OWNER;
				 }
				 elseif(preg_match('/[^\d]/',$HTTP_POST_VARS['bank_account'])) {
				   $error = MODULE_PAYMENT_NOVALNET_ELV_DE_TEXT_JS_BANK_ERROR_ACCOUNT_NUMBER;
				 }
				elseif(!$HTTP_POST_VARS['bank_account'] || strlen($HTTP_POST_VARS['bank_account'])<MODULE_PAYMENT_NOVALNET_ELV_DE_TEXT_BANK_ACCOUNT_NUMBER_LENGTH){
				$error = MODULE_PAYMENT_NOVALNET_ELV_DE_TEXT_JS_BANK_ACCOUNT_NUMBER;
				}
				elseif(preg_match('/[^\d]/',$HTTP_POST_VARS['bank_code'])) { 
				$error = MODULE_PAYMENT_NOVALNET_ELV_DE_TEXT_JS_BANK_CODE_ERROR;
				}
				elseif(!$HTTP_POST_VARS['bank_code'] || strlen($HTTP_POST_VARS['bank_code'])<MODULE_PAYMENT_NOVALNET_ELV_DE_TEXT_BANK_CODE_LENGTH){
				$error = MODULE_PAYMENT_NOVALNET_ELV_DE_TEXT_JS_BANK_CODE;
				}
				elseif(MODULE_PAYMENT_NOVALNET_ELV_DE_ACDC=="True" && !$HTTP_POST_VARS['acdc']) $error .= MODULE_PAYMENT_NOVALNET_ELV_DE_TEXT_JS_ACDC;
				...
				if (isset($HTTP_POST_VARS['user_email_elv_de']))
				{ $_SESSION['user_email_elv_de']  = $HTTP_POST_VARS['user_email_elv_de']; }
				...
				if( $this->isActivatedCallback && in_array($billing_iso_code, $this->nnelvde_allowed_pin_country_list) && $amount_check >= MODULE_PAYMENT_NOVALNET_ELV_DE_PIN_BY_CALLBACK_MIN_LIMIT ){
					...
					if($error!='') {
							...
							if (isset($HTTP_POST_VARS['user_email_elv_de']))
							{
								$error_msg = MODULE_PAYMENT_NOVALNET_ELV_DE_EMAIL_INPUT_REQUEST_DESC;
							}
							else
							{
								$error_msg = MODULE_PAYMENT_NOVALNET_ELV_DE_PIN_INPUT_REQUEST_DESC ;
							}
  ...
  public function findTotalAmount(){
	...
	public function secondCall()
	{
		...
		$xml = '<?xml version="1.0" encoding="UTF-8"?>
			    ...
			    $xml .= '<pin>'.$_SESSION['novalnet_pin_elv_de'].'</pin>';$xml .= '
		  			</info_request>
				</nnxml>'; 	
		...
		{	
		   if($xml_response->status == '0529005'){
				 $_SESSION['invalid_count_de'] = $_SESSION['invalid_count_de'] + 1;
				if($_SESSION['invalid_count_de'] == 3)	{
				$payment_error_return = 'payment_error='.$this->code.'&error='.utf8_encode(MODULE_PAYMENT_NOVALNET_ELV_DE_MAX_TIME_ERROR);
				}
				else { $payment_error_return = 'payment_error='.$this->code.'&error='.utf8_encode($xml_response->status_message);}
				zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, $payment_error_return, 'SSL', true, false));	
			}
		    else
		    {
  ...
  function confirmation() {
	...
	if(count($HTTP_POST_VARS)==0 || $HTTP_POST_VARS=='') $HTTP_POST_VARS = $_POST;
	 ...
	 $cardnoInfo = $_SESSION['bank_account'];
	 $codeInfo = $_SESSION['bank_code'];
	 
	 if($cardnoInfo) {
		$cardnoInfo=str_pad(substr($cardnoInfo,0,-4),strlen($cardnoInfo),'*',STR_PAD_RIGHT);
	}
	if($codeInfo) {
		$codeInfo=str_pad(substr($codeInfo,0,-4),strlen($codeInfo),'*',STR_PAD_RIGHT);
	}
  ...
  // @return string
    ...
    $process_button_string = zen_draw_hidden_field('bank_account_holder', $_SESSION['bank_account_holder']) .
                             zen_draw_hidden_field('bank_account', $_SESSION['bank_account']) .
                             zen_draw_hidden_field('bank_code', $_SESSION['bank_code']) .
			     zen_draw_hidden_field('acdc', $_SESSION['acdc']);
  ...
  function before_process() {
	...
	$billing_iso_code = strtolower($order->customer['country']['iso_code_2']);
	if(count($HTTP_POST_VARS)==0 || $HTTP_POST_VARS=='') $HTTP_POST_VARS = $_POST;
	$_SESSION['nn_amount_elv_de'] = $this->findTotalAmount();
	...
	{	
		...
		if($aryResponse = $this->secondCall()){
			...
			if($_SESSION['test_mode_elv_de'] == 1 || $test_mode )
				$order->info['comments'] .= '<B><br>'.MODULE_PAYMENT_NOVALNET_ELV_DE_TEST_ORDER_MESSAGE.'</B>';	
			$order->info['comments'] .= '<B><br>'.MODULE_PAYMENT_NOVALNET_ELV_DE_TID_MESSAGE.$_SESSION['nn_tid_elv_de'].'</B><br />';
	...
	//$amount = $this->findTotalAmount();
	$amount = $_SESSION['nn_amount_elv_de'];
	...
	// set post params
	if( $this->isActivatedCallback && in_array($billing_iso_code, $this->nnelvde_allowed_pin_country_list) && $amount >= MODULE_PAYMENT_NOVALNET_ELV_DE_PIN_BY_CALLBACK_MIN_LIMIT ){			
		...
		$customer_no = ($customer['customers_status'] != 1) ? $nn_customer_id : MODULE_PAYMENT_NOVALNET_ELV_DE_GUEST_USER;
    ...
    $urlparam .= '&bank_account_holder='.utf8_encode($HTTP_POST_VARS['bank_account_holder']).'&bank_account='.$HTTP_POST_VARS['bank_account'];
    $urlparam .= '&bank_code='.$HTTP_POST_VARS['bank_code'].'&first_name='.$firstname.'&last_name='.$lastname;
    $urlparam .= '&street='.$street_address.'&city='.$city.'&zip='.$postcode;
    $urlparam .= '&country='.$country_iso_code_2.'&email='.$email_address;
    $urlparam .= '&search_in_street=1'.'&tel='.$user_telephone.'&remote_ip='.$user_ip.$acdc;
	...
	if($aryResponse['status']==100)
    {
		...
		if( $this->isActivatedCallback && in_array($billing_iso_code, $this->nnelvde_allowed_pin_country_list) && $amount >= MODULE_PAYMENT_NOVALNET_ELV_DE_PIN_BY_CALLBACK_MIN_LIMIT ){
			...
			$order->info['comments'] .= '<B><br>'.MODULE_PAYMENT_NOVALNET_ELV_DE_TID_MESSAGE.$aryResponse['tid'].'</B><br />';
			$_SESSION['nn_tid_elv_de'] = $aryResponse['tid'];
			$order->info['comments']  = str_replace(array('<b>', '</b>','<B>','</B>', '<br>','<br />','<BR>'), array('', '', '','',"\n", "\n","\n"), $order->info['comments']);	
		}
    }
  ...
  function after_process() {
    ...
    $urlparam .= '&auth_code='.$auth_code.'&status=100&tid='.$_SESSION['nn_tid_elv_de'].'&vwz3='.$insert_id.'&vwz3_prefix='.MODULE_PAYMENT_NOVALNET_ELV_DE_TEXT_ORDERNO.'&vwz4='.date('Y.m.d').'&vwz4_prefix='.MODULE_PAYMENT_NOVALNET_ELV_DE_TEXT_ORDERDATE.'&order_no='.$insert_id;
	...
	unset($_SESSION['nn_amount_elv_de']);	
	unset($_SESSION['user_email_elv_de']);
	unset($_SESSION['email_reply_check_elv_de']);
	unset($_SESSION['new_novalnet_pin_elv_de']);
	unset($_SESSION['elv_de_order_amount']);
    if(isset($_SESSION['invalid_count_de']))
	{
		unset($_SESSION['invalid_count_de']);
  ...
  function install() {
	...
	$acdccontrol_title = $this->install_lang('acdccontrol_title', DEFAULT_LANGUAGE);
    $acdccontrol_desc = $this->install_lang('acdccontrol_desc', DEFAULT_LANGUAGE);
	
	/*$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$allowed_title."','MODULE_PAYMENT_NOVALNET_ELV_DE_ALLOWED', '','".$allowed_desc."', '6', '0', now())"); */ 
   
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('".$enable_title."', 'MODULE_PAYMENT_NOVALNET_ELV_DE_STATUS', 'True', '".$enable_desc."', '6', '0', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
	...
	$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$amountlimitpin_title."','MODULE_PAYMENT_NOVALNET_ELV_DE_PIN_BY_CALLBACK_MIN_LIMIT', '','".$amountlimitpin_desc."', '6', '2', now())");	
	
	$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('".$test_title."', 'MODULE_PAYMENT_NOVALNET_ELV_DE_TEST_MODE', 'True', '".$test_desc."', '6', '3', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");	
	
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$vendor_title."', 'MODULE_PAYMENT_NOVALNET_ELV_DE_VENDOR_ID', '', '".$vendor_desc."', '6', '4', now())");
	
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$auth_title."', 'MODULE_PAYMENT_NOVALNET_ELV_DE_AUTH_CODE', '', '".$auth_desc."', '6', '5', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$product_title."', 'MODULE_PAYMENT_NOVALNET_ELV_DE_PRODUCT_ID', '', '".$product_desc."', '6', '6', now())");
	
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$tariff_title."', 'MODULE_PAYMENT_NOVALNET_ELV_DE_TARIFF_ID', '', '".$tariff_desc."', '6', '7', now())");
	
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$booking_title."', 'MODULE_PAYMENT_NOVALNET_ELV_DE_MANUAL_CHECK_LIMIT', '', '".$booking_desc."', '6', '8', now())");
	
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$secondproduct_title."', 'MODULE_PAYMENT_NOVALNET_ELV_DE_PRODUCT_ID2', '', '".$secondproduct_desc."', '6', '9', now())");
	
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$secondtariff_title."', 'MODULE_PAYMENT_NOVALNET_ELV_DE_TARIFF_ID2', '', '".$secondtariff_desc."', '6', '10', now())");
	
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$enduser_title."', 'MODULE_PAYMENT_NOVALNET_ELV_DE_INFO', '', '".$enduser_desc."', '6', '11', now())");
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$sortorder_title."', 'MODULE_PAYMENT_NOVALNET_ELV_DE_SORT_ORDER', '0', '".$sortorder_desc."', '6', '12', now())");
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('".$paymnetzone_title."', 'MODULE_PAYMENT_NOVALNET_ELV_DE_ZONE', '0', '".$paymnetzone_desc."', '6', '14', 'zen_get_zone_class_title', 'zen_cfg_pull_down_zone_classes(', now())");
	
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$proxy_title."', 'MODULE_PAYMENT_NOVALNET_ELV_DE_PROXY', '', '".$proxy_desc."', '6', '15', now())");	
	
	$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('".$logo_title."', 'MODULE_PAYMENT_ELV_DE_LOGO_STATUS', 'True', '".$logo_desc."', '6', '16', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
