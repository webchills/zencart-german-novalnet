...
#                                                       #
# Version : novalnet_invoice.php vzenIN1.3.1 2009-03-01 #
#                                                       #
#########################################################
...
class novalnet_invoice {
  ...
  function selection() {
	...
	if(!$_SESSION['nn_tid_invoice']){
		...
		if($this->isActivatedCallback && strtolower($order->customer['country']['iso_code_2']) == 'de' && $amount_check >= MODULE_PAYMENT_NOVALNET_INVOICE_PIN_BY_CALLBACK_MIN_LIMIT )	
		{
			$selection['fields'][] = array( 'title' => MODULE_PAYMENT_NOVALNET_INVOICE_PIN_BY_CALLBACK_SMS_TEL, 'field' => zen_draw_input_field ( 'user_tel_invoice', $order->customer['telephone'], 'id="'.$this->code.'-callback" '.$onFocus ) );
		}
	}else{
		$selection = array('id' => $this->code,
						   'module' => $this->public_title,
		...
		$selection['fields'][] = array( 'title' => MODULE_PAYMENT_NOVALNET_INVOICE_PIN_INPUT_REQUEST_DESC, 'field' => zen_draw_input_field( 'novalnet_pin_invoice', '', 'id="'.$this->code.'-callback" '.$onFocus.' maxlength="4" size="4" ') );
  ...
  function pre_confirmation_check() {
	...
	if($_SESSION['nn_tid_invoice'])
    {
		...
		if($_SESSION['invoice_order_amount'] != $amount){
			...
			$payment_error_return = 'payment_error=' . $this->code;
			$messageStack->add_session('checkout_payment', MODULE_PAYMENT_NOVALNET_INVOICE_AMOUNT_VARIATION_MESSAGE . '<!-- ['.$this->code.'] -->', 'error');						
			zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, $payment_error_return, 'SSL', true, false));			
		}	
			// check pin
		...
		$error = '';		
		if(!MODULE_PAYMENT_NOVALNET_INVOICE_VENDOR_ID || !MODULE_PAYMENT_NOVALNET_INVOICE_AUTH_CODE || !MODULE_PAYMENT_NOVALNET_INVOICE_PRODUCT_ID || 
		!MODULE_PAYMENT_NOVALNET_INVOICE_TARIFF_ID)
		{
  ...
  function confirmation() {
    ...
    $confirmation = array('fields' => array(array( 'field' => MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_BANK_INFO)));
  ...
  function before_process() {
	...
	{	
		...
		if($_SESSION['test_mode_invoice'] ==  1){
		  ...
		  {
			$order->info['comments'] .= '<BR><B>'.MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_DURATION_LIMIT_INFO.' '.$_SESSION['due_date_invoice'].' '.MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_DURATION_LIMIT_END_INFO.".</B><BR><BR>";
		  }
		  else
		  {
			$order->info['comments'] .= '<BR><B>'.MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_TRANSFER_INFO.'</B><BR><BR>';
		  }
		  $order->info['comments'] .= MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_BANK_ACCOUNT_OWNER.' NOVALNET AG<BR>';
		  $order->info['comments'] .= MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_BANK_ACCOUNT_NUMBER.' '.$_SESSION['nn_invoice_account'].'<BR>';
		  $order->info['comments'] .= MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_BANK_CODE.' '.$_SESSION['nn_invoice_bankcode'].'<BR>';
		  $order->info['comments'] .= MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_BANK_BANK.' '.$_SESSION['nn_invoice_bankname'].', 
		  '.$_SESSION['nn_invoice_bankplace'].'<BR><BR>';
		  $order->info['comments'] .= MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_BANK_IBAN.' '.$_SESSION['nn_invoice_iban'].'<BR>';
		  $order->info['comments'] .= MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_BANK_BIC.' '.$_SESSION['nn_invoice_bic'].'<BR><BR>';
		  $order->info['comments'] .= MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_AMOUNT.' '.$_SESSION['original_amount_invoice'].' '.$order->info['currency'].'<BR>';    
		  $order->info['comments'] .= MODULE_PAYMENT_NOVALNET_INVOICE_TID_MESSAGE.'<B>'.$_SESSION['nn_tid_invoice'].'</B><BR>';
		  $order->info['comments'] .= '<B>'.MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_REFERENCE_INFO.'</B><BR>';
    ...
    $product_id = MODULE_PAYMENT_NOVALNET_INVOICE_PRODUCT_ID;
    $tariff_id = MODULE_PAYMENT_NOVALNET_INVOICE_TARIFF_ID;
	...
	else
	{
		$user_telephone	= $order->customer['telephone'];
			...
			// set post params
	if( $this->isActivatedCallback && strtolower($order->customer['country']['iso_code_2']) == 'de' && $amount >= MODULE_PAYMENT_NOVALNET_INVOICE_PIN_BY_CALLBACK_MIN_LIMIT )
	{			
    ...
    $urlparam  = 'vendor='.MODULE_PAYMENT_NOVALNET_INVOICE_VENDOR_ID.'&product='.$product_id.'&key=27&tariff='.$tariff_id;
    $urlparam .= '&auth_code='.MODULE_PAYMENT_NOVALNET_INVOICE_AUTH_CODE.'&currency='.$order->info['currency'];
    ...
    {
		if( $this->isActivatedCallback && strtolower($order->customer['country']['iso_code_2']) == 'de' && $amount >= MODULE_PAYMENT_NOVALNET_INVOICE_PIN_BY_CALLBACK_MIN_LIMIT ){ 			  $_SESSION['invoice_order_amount']= $amount;
			...
			$_SESSION['nn_invoice_bankplace'] = $aryResponse['invoice_bankplace'];			
			 ...
			 if($aryResponse['test_mode'] == 1){
					$order->info['comments'] .= MODULE_PAYMENT_NOVALNET_INVOICE_TEST_ORDER_MESSAGE;
			  }elseif(MODULE_PAYMENT_NOVALNET_INVOICE_TEST_MODE == 'Yes' or MODULE_PAYMENT_NOVALNET_INVOICE_TEST_MODE == '1'){
					$order->info['comments'] .= MODULE_PAYMENT_NOVALNET_INVOICE_TEST_ORDER_MESSAGE;
			  }
			  ...
			  if($due_date)
			  {
				$order->info['comments'] .= '<BR><B>'.MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_DURATION_LIMIT_INFO." $due_date 
			".MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_DURATION_LIMIT_END_INFO.".</B><BR><BR>";
			  }
			  else
			  {
				$order->info['comments'] .= '<BR><B>'.MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_TRANSFER_INFO.'</B><BR><BR>';
			  ...
			  $order->info['comments'] .= MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_BANK_ACCOUNT_OWNER.' NOVALNET AG<BR>';
			  $order->info['comments'] .= MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_BANK_ACCOUNT_NUMBER.' '.$aryResponse['invoice_account'].'<BR>';
			  $order->info['comments'] .= MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_BANK_CODE.' '.$aryResponse['invoice_bankcode'].'<BR>';
			  $order->info['comments'] .= MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_BANK_BANK.' '.$aryResponse['invoice_bankname'].', 
			  '.$aryResponse['invoice_bankplace'].'<BR><BR>';
			  $order->info['comments'] .= MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_BANK_IBAN.' '.$aryResponse['invoice_iban'].'<BR>';
			  $order->info['comments'] .= MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_BANK_BIC.' '.$aryResponse['invoice_bic'].'<BR><BR>';
			  $order->info['comments'] .= MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_AMOUNT.' '.$_SESSION['nn_total'].' '.$order->info['currency'].'<BR>';    
			  $order->info['comments'] .= MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_REFERENCE.' <B>TID '.$aryResponse['tid'].'</B><BR>';
			  $order->info['comments'] .= '<B>'.MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_REFERENCE_INFO.'</B><BR>';
			  $order->info['comments'] .= $old_comments;
			  $order->info['comments']  = str_replace(array('<B>', '</B>', '<BR>', '<br />','<b>','</b>','<br>'), array('', '', "\n", "\n"), $order->info['comments']);		
			  $_SESSION['nn_tid_invoice'] = $aryResponse['tid'];
  ...
  public function findTotalAmount(){
  	...
  	public function secondCall()
	{
		...
		$xml = '<?xml version="1.0" encoding="UTF-8"?>
				<nnxml>                               
		  			<info_request>
			    		<vendor_id>'.MODULE_PAYMENT_NOVALNET_INVOICE_VENDOR_ID.'</vendor_id>
			    		<vendor_authcode>'.MODULE_PAYMENT_NOVALNET_INVOICE_AUTH_CODE.'</vendor_authcode>
		...
		{		
			$payment_error_return = 'payment_error='.$this->code;
			$messageStack->add_session('checkout_payment', $xml_response->status_message . '<!-- ['.$this->code.'] -->', 'error');						
			zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, $payment_error_return, 'SSL', true, false));
		}
		else
		{             	
  ...
  function after_process() {
	...
	if($_SESSION['nn_tid_invoice'] != ''){
		...
		$url = 'https://payport.novalnet.de/paygate.jsp';
		$urlparam = 'vendor='.MODULE_PAYMENT_NOVALNET_INVOICE_VENDOR_ID.'&product='.$product_id.'&key=27&tariff='.$tariff_id;
		$urlparam .= '&auth_code='.MODULE_PAYMENT_NOVALNET_INVOICE_AUTH_CODE.'&status=100&tid='.$_SESSION['nn_tid_invoice'].'&reference=BNR-'.$insert_id.'&vwz2='.MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_ORDERNO.''.$insert_id.'&vwz3='.MODULE_PAYMENT_NOVALNET_INVOICE_TEXT_ORDERDATE.''.date('Y-m-d H:i:s');
...
*/
  ...
  function install() {
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Allowed zones','MODULE_PAYMENT_NOVALNET_INVOICE_ALLOWED', '','Please enter the zones <b>separately</b> which should be allowed to use this modul (e. g. AT,DE (leave empty if you want to allow all zones))', '6', '0', now())");
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable Test Mode', 'MODULE_PAYMENT_NOVALNET_INVOICE_TEST_MODE', 'True', 'Do you want to enable the test mode?', '6', '2', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Please contact sales@novalnet.de if you do not have any of the following Novalnet IDs!<BR><P>Wenn Sie keine oder irgendeine der folgenden Novalnet IDs nicht haben sollten, bitte sich an sales@novalnet.de wenden!<BR><P>Novalnet Merchant ID', 'MODULE_PAYMENT_NOVALNET_INVOICE_VENDOR_ID', '', 'Your Merchant ID of Novalnet', '6', '3', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Novalnet Authorisation Code', 'MODULE_PAYMENT_NOVALNET_INVOICE_AUTH_CODE', '', 'Your Authorisation Code of Novalnet', '6', '4', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Novalnet Product ID', 'MODULE_PAYMENT_NOVALNET_INVOICE_PRODUCT_ID', '', 'Your Product ID of Novalnet', '6', '5', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Novalnet Tariff ID', 'MODULE_PAYMENT_NOVALNET_INVOICE_TARIFF_ID', '', 'Your Tariff ID of Novalnet', '6', '6', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Invoice Duration', 'MODULE_PAYMENT_NOVALNET_INVOICE_DURATION', '', 'Invoice Duration', '6', '7', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Information to the Customer', 'MODULE_PAYMENT_NOVALNET_INVOICE_INFO', '','will be shown on the payment formula', '6', '8', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort order of display.', 'MODULE_PAYMENT_NOVALNET_INVOICE_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '9', now())");
  ...
  // @return array
  function keys() {
    return array('MODULE_PAYMENT_NOVALNET_INVOICE_ALLOWED','MODULE_PAYMENT_NOVALNET_INVOICE_PIN_BY_CALLBACK_SMS','MODULE_PAYMENT_NOVALNET_INVOICE_PIN_BY_CALLBACK_MIN_LIMIT', 'MODULE_PAYMENT_NOVALNET_INVOICE_STATUS', 'MODULE_PAYMENT_NOVALNET_INVOICE_TEST_MODE', 
