...
#                                                       #
#  Version : novalnet_elv_at_pci.php 1.0.0 2010-06-16   #
#                                                       #
#########################################################
...
class novalnet_elv_at_pci 
{
  ...
  function novalnet_elv_at_pci(){
		global $order;
		$this->key			= MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_PASSWORD;
  ...
  function process_button() {
		...
		$tariff_id = MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TARIFF_ID;
		$manual_check_limit = trim(MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_MANUAL_CHECK_LIMIT);
		$manual_check_limit = str_replace(',', '', $manual_check_limit);
		$manual_check_limit = str_replace('.', '', $manual_check_limit);

		if($manual_check_limit && $amount>=$manual_check_limit){
			$product_id = MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_PRODUCT_ID2;
			$tariff_id = MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TARIFF_ID2;
		}
		...
		list($auth_code, $product_id, $tariff_id, $amount, $test_mode, $uniqid, $hash) = $this->encodeParams($auth_code=MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_AUTH_CODE, $product_id, $tariff_id, $amount, $test_mode, $uniqid);
		

	    $process_button_string =   zen_draw_hidden_field('vendor_id', MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_VENDOR_ID) .
								   zen_draw_hidden_field('product_id', $product_id) .
								   zen_draw_hidden_field('payment_id', '8') .
								   zen_draw_hidden_field('tariff_id', $tariff_id) .
								   zen_draw_hidden_field('vendor_authcode', MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_AUTH_CODE) .
 ...
 ### Send the order detail to Novalnet ###
  function after_process(){
		...
		if($_POST['tid'] != ''){
			...
			$urlparam = 'vendor='.MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_VENDOR_ID.'&product='.$product_id.'&key=6&tariff='.$tariff_id;
			$urlparam .= '&auth_code='.MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_AUTH_CODE.'&status=100&tid='.$_POST['tid'].'&reference=BNR-'.$insert_id.'&vwz2='.MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TEXT_ORDERNO.''.$insert_id.'&vwz3='.MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TEXT_ORDERDATE.''.date('Y-m-d H:i:s');
