...
class novalnet_elv_at_pci 
{
  ...
  function novalnet_elv_at_pci(){
		...
		$this->key          = trim(MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_PASSWORD); #'z2Vw3E4j';
		$this->vendor_id  = trim(MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_VENDOR_ID);
		$this->auth_code  = trim(MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_AUTH_CODE);
		$this->product_id = trim(MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_PRODUCT_ID);
		$this->tariff_id  = trim(MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TARIFF_ID);
		$this->manual_check_limit = trim(MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_MANUAL_CHECK_LIMIT);
		$this->product_id2 = trim(MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_PRODUCT_ID2);
		$this->tariff_id2 = trim(MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TARIFF_ID2);
		$this->test_mode = (strtolower(MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TEST_MODE) == 'true' or MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TEST_MODE == '1')? 1: 0;
		
		$this->code         = 'novalnet_elv_at_pci';
		$this->title        = MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TEXT_TITLE;
		...
		if(MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_LOGO_STATUS == 'True'){
			$this->public_title = 'Novalnet'.' '.MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TEXT_PUBLIC_TITLE;
			$this->title        = 'Novalnet'.' '.MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TEXT_TITLE;
		}
		...
		if($_POST['session'] && $_SESSION['payment'] == $this->code){$this->checkSecurity();}
		
 }
  
  function checkConfigure() {
		if (IS_ADMIN_FLAG == true) {
			$this->title = MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TEXT_TITLE; // Payment module title in Admin
			if(MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_LOGO_STATUS == 'True'){
				$this->public_title = 'Novalnet'.' '.MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TEXT_PUBLIC_TITLE;
				$this->title        = 'Novalnet'.' '.MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TEXT_TITLE;
			}
			if ($this->enabled == 'true' && (!$this->vendor_id || !$this->auth_code || !$this->product_id || !$this->tariff_id || !$this->key )) {
				$this->title .=  '<span class="alert">'.MODULE_PAYMENT_NOVALNET_AT_PCI_NOT_CONFIGURED.'</span>';
			} elseif ($this->test_mode == '1') {
				$this->title .= '<span class="alert">'.MODULE_PAYMENT_NOVALNET_AT_PCI_IN_TEST_MODE.'</span>';
			}
  ...
  function update_status() {
		...
		if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_ZONE > 0) ) {
			  $check_flag = false;
			  $check = $db->Execute("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
			  while (!$check->EOF) {
		...
		if(count($HTTP_POST_VARS)==0 || $HTTP_POST_VARS=='') $HTTP_POST_VARS = $_POST;
			...
			$selection = array('id' => $this->code,
                       'module' => $this->public_title,
                        'fields' => array(array('title' => '', 'field' => MODULE_PAYMENT_NOVALNET_INFORMATION_PAYMENT_AT_PCI),
										  array('title' => '', 'field' => MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_BOOK_REF)
						));
  ...
  function process_button() {
		...
		if (preg_match('/[^\d\.]/', $amount) or !$amount){
			...
			$_SESSION['nn_amount_elv_at_pci'] = $amount;
		...
		if(strstr($checkout_url, '?')){
			  ...
			  if(substr($checkout_url,-1)=='?')$error_url = $checkout_url.'payment_error=novalnet_elv_at_pci&error=$ERROR_MESSAGE ($STATUS)';
			  else $error_url = $checkout_url.'&payment_error=novalnet_elv_at_pci&error=$ERROR_MESSAGE ($STATUS)';
		}
		else $error_url = $checkout_url.'?payment_error=novalnet_elv_at_pci&error=$ERROR_MESSAGE ($STATUS)';
		...
		$customer_no = ($customer['customers_status'] != 1) ? $nn_customer_id : MODULE_PAYMENT_NOVALNET_AT_PCI_GUEST_USER;
		
		
	    $process_button_string = zen_draw_hidden_field('vendor_id', $vendor_id) .
								   zen_draw_hidden_field('product_id', $product_id) .
								   zen_draw_hidden_field('payment_id',  $this->payment_key) .
								   ...
								   zen_draw_hidden_field('lang', MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TEXT_LANG) .
								   zen_draw_hidden_field('language', MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TEXT_LANG) .
  ...
  function before_process() {
		...
		if($_POST['tid'] && $_POST['status'] == '100'){
			...
			if ($_POST['test_mode'] == 1){
				$order->info['comments'] .= '<B><br>'.MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TEST_ORDER_MESSAGE.'</B>';
			}

			$order->info['comments'] .=  '<B><br>'.MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TID_MESSAGE.$_POST['tid'].'</B><br />';
 ...
 ### Send the order detail to Novalnet ###
  function after_process(){
    ...
    if($manual_check_limit && $_SESSION['nn_amount_elv_at_pci']>=$manual_check_limit)
    {
      $product_id  =  $this->product_id2;;
      $tariff_id   =  $this->tariff_id2;
    }
		...
		if($_POST['tid'] != ''){
			...
			$urlparam .= '&auth_code='.$auth_code.'&status=100&tid='.$_POST['tid'].'&vwz2='.MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TEXT_ORDERNO.''.$insert_id.'&vwz3='.MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TEXT_ORDERDATE.''.date('Y-m-d H:i:s').'&order_no='.$insert_id;
		...
		unset($_SESSION['nn_amount_elv_at_pci']);
		return false;
  }
  ...
  function get_error()
  {
		...
		$error = array('title' => MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TEXT_ERROR, 'error' => stripslashes(utf8_decode($HTTP_GET_VARS['error'])));
		return $error;
  }
  ...
  function check() 
  {
		...
		{
		  $check_query = $db->Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_STATUS'");
  ...
  function install() {
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Allowed zones', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_ALLOWED', '', 'Please enter the zones <b>separately</b> which should be allowed to use this modul (e. g. AT,DE (leave empty if you want to allow all zones))', '6', '0', now())"); 
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable Test Mode', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TEST_MODE', 'True', 'Do you want to enable the test mode?', '6', '2', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
    
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Please contact sales@novalnet.de if you do not have any of the following Novalnet IDs!<BR><P>Wenn Sie keine oder irgendeine der folgenden Novalnet IDs nicht haben sollten, bitte sich an sales@novalnet.de wenden!<BR><P>Novalnet Merchant ID', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_VENDOR_ID', '', 'Your Merchant ID of Novalnet', '6', '3', now())");
    
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Novalnet Authorisation Code', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_AUTH_CODE', '', 'Your Authorisation Code of Novalnet', '6', '4', now())");

    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Novalnet Product ID', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_PRODUCT_ID', '', 'Your Product ID of Novalnet', '6', '5', now())");
    
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Novalnet Tariff ID', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TARIFF_ID', '', 'Your Tariff ID of Novalnet', '6', '6', now())");
    
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Manual checking amount in cents', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_MANUAL_CHECK_LIMIT', '', 'Please enter the amount in cents', '6', '7', now())"); 

    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Your second Product ID in Novalnet', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_PRODUCT_ID2', '', 'for the manual checking', '6', '8', now())"); 

    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('The Tariff ID of the second product', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TARIFF_ID2', '', 'for the manual checking', '6', '9', now())");
    
	 $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Password please', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_PASSWORD', '', 'for the manual checking', '6', '10', now())");
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort order of display.', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '12', now())");
	...
	/*$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$allowed_title."','MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_ALLOWED', '','".$allowed_desc."', '6', '0', now())"); */
   
  	$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('".$enable_title."', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_STATUS', 'True', '".$enable_desc."', '6', '0', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$vendor_title."', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_VENDOR_ID', '', '".$vendor_desc."', '6', '3', now())");
	
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$auth_title."', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_AUTH_CODE', '', '".$auth_desc."', '6', '4', now())");
	
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$product_title."', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_PRODUCT_ID', '', '".$product_desc."', '6', '5', now())");
	
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$tariff_title."', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TARIFF_ID', '', '".$tariff_desc."', '6', '6', now())");
	
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$booking_title."', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_MANUAL_CHECK_LIMIT', '', '".$booking_desc."', '6', '7', now())");
	
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$secondproduct_title."', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_PRODUCT_ID2', '', '".$secondproduct_desc."', '6', '8', now())");
	
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$secondtariff_title."', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TARIFF_ID2', '', '".$secondtariff_desc."', '6', '9', now())");
	
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$enduser_title."', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_BOOK_REF', '', '".$enduser_desc."', '6', '10', now())");   
	
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$sortorder_title."', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_SORT_ORDER', '0', '".$sortorder_desc."', '6', '11', now())");
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('".$paymnetzone_title."', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_ZONE', '0', '".$paymnetzone_desc."', '6', '13', 'zen_get_zone_class_title', 'zen_cfg_pull_down_zone_classes(', now())");
	
	$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$password_title."', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_PASSWORD', '', '".$password_desc."', '6', '14', now())");	
	
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$proxy_title."', 'MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_PROXY', '', '".$proxy_desc."', '6', '15', now())"); 	
    ...
    function checkReturnedData(){
  	 if ($_POST['hash2'] && $_SESSION['payment'] == $this->code){
			if (strtoupper($this->implementation) == 'JAVA_PCI'){
				#Java encoded
				if( $_POST['vendor_authcode'] != md5(MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_AUTH_CODE.strrev($this->key)) ){
					  $err = MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_TEXT_HASH_ERROR.'; wrong auth_code!';
					  $payment_error_return = 'payment_error=novalnet_elv_at_pci&error='.$_POST['status_text'].'; '.$err;
					  zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, $payment_error_return, 'SSL', true, false));
				}
				$_POST['auth_code']  = MODULE_PAYMENT_NOVALNET_ELV_AT_PCI_AUTH_CODE;#todo: check?
