...
class novalnet_cc_pci {
	...
	function novalnet_cc_pci(){ 
		...
		$this->key          = trim(MODULE_PAYMENT_NOVALNET_CC_PCI_PASSWORD); #'z2Vw3E4j';
		$this->vendor_id    = trim(MODULE_PAYMENT_NOVALNET_CC_PCI_VENDOR_ID);
		$this->auth_code    = trim(MODULE_PAYMENT_NOVALNET_CC_PCI_AUTH_CODE);
		$this->product_id   = trim(MODULE_PAYMENT_NOVALNET_CC_PCI_PRODUCT_ID);
		$this->tariff_id    = trim(MODULE_PAYMENT_NOVALNET_CC_PCI_TARIFF_ID);
		$this->manual_check_limit = trim(MODULE_PAYMENT_NOVALNET_CC_PCI_MANUAL_CHECK_LIMIT);
		$this->product_id2 = trim(MODULE_PAYMENT_NOVALNET_CC_PCI_PRODUCT_ID2);
		$this->tariff_id2  = trim(MODULE_PAYMENT_NOVALNET_CC_PCI_TARIFF_ID2);
		$this->test_mode = (strtolower(MODULE_PAYMENT_NOVALNET_CC_PCI_TEST_MODE) == 'true' or MODULE_PAYMENT_NOVALNET_CC_PCI_TEST_MODE == '1')? 1: 0;
		...
		if(MODULE_PAYMENT_NOVALNET_CC_PCI_LOGO_STATUS == 'True'){
			$this->public_title = 'Novalnet'.' '.MODULE_PAYMENT_NOVALNET_CC_PCI_TEXT_PUBLIC_TITLE;
			$this->title        = 'Novalnet'.' '.MODULE_PAYMENT_NOVALNET_CC_PCI_TEXT_TITLE;
		}
		...
		if (IS_ADMIN_FLAG == true) {
			$this->title = MODULE_PAYMENT_NOVALNET_CC_PCI_TEXT_TITLE; // Payment module title in Admin
			if(MODULE_PAYMENT_NOVALNET_CC_PCI_LOGO_STATUS == 'True'){
				$this->public_title = 'Novalnet'.' '.MODULE_PAYMENT_NOVALNET_CC_PCI_TEXT_PUBLIC_TITLE;
				$this->title        = 'Novalnet'.' '.MODULE_PAYMENT_NOVALNET_CC_PCI_TEXT_TITLE;
			}
			if ($this->enabled == 'true' && (!$this->vendor_id || !$this->auth_code || !$this->product_id || !$this->tariff_id || !$this->key )) {
				$this->title .=  '<span class="alert">'.MODULE_PAYMENT_NOVALNET_CC_PCI_NOT_CONFIGURED.'</span>';
			} elseif ($this->test_mode == '1') {
				$this->title .= '<span class="alert">'.MODULE_PAYMENT_NOVALNET_CC_PCI_IN_TEST_MODE.'</span>';
			}
  ...
  function update_status() {
    global $order, $db;
		...
		if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_NOVALNET_CC_PCI_ZONE > 0) ){
			$check_flag = false;
			$check = $db->Execute("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_NOVALNET_CC_PCI_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
  ...
  function selection(){
		...
		$selection = array('id' => $this->code,
						   ...
						   'fields' => array(array('title' => '', 'field' => MODULE_PAYMENT_NOVALNET_INFORMATION_PAYMENT_CC_PCI),
										 array('title' => '', 'field' => MODULE_PAYMENT_NOVALNET_CC_PCI_BOOK_REF)
						   ));
  ...
  function process_button(){
		...
		$_SESSION['nn_amount_ccpci'] = $amount;
	   ...
	   $process_button_string = zen_draw_hidden_field('vendor_id', $vendor_id) .
							    ...
							    #zen_draw_hidden_field('house_no', '2') .
							    ...
							    zen_draw_hidden_field('lang', MODULE_PAYMENT_NOVALNET_CC_PCI_TEXT_LANG) .
							    zen_draw_hidden_field('language', MODULE_PAYMENT_NOVALNET_CC_PCI_TEXT_LANG) .
  ...
  function before_process(){
		...
		if($_POST['tid'] && $_POST['status'] == '100'){
			...
			if ($_POST['test_mode'] == 1){
				$order->info['comments'] .= '<B><br>'.MODULE_PAYMENT_NOVALNET_CC_PCI_TEST_ORDER_MESSAGE.'</B>';
			}
			$order->info['comments'] .= '<B><br>'.MODULE_PAYMENT_NOVALNET_CC_PCI_TID_MESSAGE.$_POST['tid'].'</B><br />';
  ...
  function after_process(){
		...
		if($manual_check_limit && $_SESSION['nn_amount_ccpci']>=$manual_check_limit)
		{
		$product_id  =  $this->product_id2;;
		$tariff_id   =  $this->tariff_id2;
		}
		if($_POST['tid'] != ''){
			...
			$urlparam .= '&auth_code='.$auth_code.'&status=100&tid='.$_POST['tid'].'&vwz2='.MODULE_PAYMENT_NOVALNET_CC_PCI_TEXT_ORDERNO.''.$insert_id.'&vwz3='.MODULE_PAYMENT_NOVALNET_CC_PCI_TEXT_ORDERDATE.''.date('Y-m-d H:i:s').'&order_no='.$insert_id;
		...
		unset($_SESSION['nn_amount_ccpci']);
	   return false;
  }
  ...
  function check() {
		...
		if (!isset($this->_check)){
			$check_query = $db->Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_NOVALNET_CC_PCI_STATUS'");
  ...
  function install() {
	...
	//$db->Execute("alter table ".TABLE_ORDERS." change payment_method payment_method text");
	  ...
	  /*$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$allowed_title."','MODULE_PAYMENT_NOVALNET_CC_PCI_ALLOWED', '','".$allowed_desc."', '6', '0', now())");*/

		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('".$enable_title."', 'MODULE_PAYMENT_NOVALNET_CC_PCI_STATUS', 'True', '".$enable_desc."', '6', '0', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
		...
		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$vendor_title."', 'MODULE_PAYMENT_NOVALNET_CC_PCI_VENDOR_ID', '', '".$vendor_desc."', '6', '2', now())");

		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$auth_title."', 'MODULE_PAYMENT_NOVALNET_CC_PCI_AUTH_CODE', '', '".$auth_desc."', '6', '3', now())");

		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$product_title."', 'MODULE_PAYMENT_NOVALNET_CC_PCI_PRODUCT_ID', '', '".$product_desc."', '6', '4', now())");

		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$tariff_title."', 'MODULE_PAYMENT_NOVALNET_CC_PCI_TARIFF_ID', '', '".$tariff_desc."', '6', '5', now())");

		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$booking_title."', 'MODULE_PAYMENT_NOVALNET_CC_PCI_MANUAL_CHECK_LIMIT', '', '".$booking_desc."', '6', '6', now())");


		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$secondproduct_title."', 'MODULE_PAYMENT_NOVALNET_CC_PCI_PRODUCT_ID2', '', '".$secondproduct_desc."', '6', '7', now())");


		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$secondtariff_title."', 'MODULE_PAYMENT_NOVALNET_CC_PCI_TARIFF_ID2', '', '".$secondtariff_desc."', '6', '8', now())");

		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$enduser_title."', 'MODULE_PAYMENT_NOVALNET_CC_PCI_BOOK_REF', '', '".$enduser_desc."', '6', '9', now())");  

		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$sortorder_title."', 'MODULE_PAYMENT_NOVALNET_CC_PCI_SORT_ORDER', '0', '".$sortorder_desc."', '6', '10', now())");
		...
		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('".$paymnetzone_title."', 'MODULE_PAYMENT_NOVALNET_CC_PCI_ZONE', '0', '".$paymnetzone_desc."', '6', '12', 'zen_get_zone_class_title', 'zen_cfg_pull_down_zone_classes(', now())");


		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$proxy_title."', 'MODULE_PAYMENT_NOVALNET_CC_PCI_PROXY', '', '".$proxy_desc."', '6', '13', now())");


		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$password_title."', 'MODULE_PAYMENT_NOVALNET_CC_PCI_PASSWORD', '', '".$password_desc."', '6', '14', now())");	
