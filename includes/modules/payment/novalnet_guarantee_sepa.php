<?php
/**
* This script is used for Guaranteed Direct Debit SEPA payment
*
* @author Novalnet AG
...
class novalnet_guarantee_sepa {
    ...
    /**
     * Function : novalnet_guarantee_sepa()
     *
     */
   ...
   /**
    ...
    function selection() {
      ...
      if (empty($order->billing['company'])) {
        ...
        $fieldsArray[] = array('title' => '', 'field' => '<div id="nn_sepa_mandate" style="cursor: pointer;"><table><tr><td class="main">'.
                    MODULE_PAYMENT_NOVALNET_GUARANTEE_SEPA_FORM_MANDATE_CONFIRM_TEXT.'</tr></td ></table></div>');
    ...
    function pre_confirmation_check() {
        ...
        if (empty($order->billing['company']) && empty($post[$this->code.'_birthdate'])) {
            $messageStack->add_session('checkout_payment', MODULE_PAYMENT_NOVALNET_GUARANTEE_SEPA_DOB_ERROR_MSG . '<!-- ['.$this->code.'] -->', 'error');
            zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL', true, false));
        }
     ...
     */
    function keys() {
        echo '<input type="hidden" id="sepa_due_date_error" value="'.MODULE_PAYMENT_SEPA_DUE_DATE_VALIDATION.'"><script src="'.DIR_WS_CATALOG.'includes/ext/novalnet/js/novalnet_admin.js" type="text/javascript"></script>';
