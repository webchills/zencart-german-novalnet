...
class novalnet_instantbanktransfer {
    ...
    function novalnet_instantbanktransfer() {
        ...
        $this->key = trim(MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_PASSWORD); #'z2Vw3E4j';
        $this->vendor_id = trim(MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_VENDOR_ID);
        $this->auth_code = trim(MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_AUTH_CODE);
        $this->product_id = trim(MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_PRODUCT_ID);
        $this->tariff_id = trim(MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_TARIFF_ID);
        $this->test_mode = (strtolower(MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_TEST_MODE) == 'true' or MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_TEST_MODE == '1') ? 1 : 0;
        ...
        if (MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_LOGO_STATUS == 'True') {
            $this->public_title = 'Novalnet'.' '. MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_TEXT_PUBLIC_TITLE;
    ...
    function checkConfigure() {
        if (IS_ADMIN_FLAG == true) {
            $this->title = MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_TEXT_TITLE; // Payment module title in Admin
			if (MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_LOGO_STATUS == 'True') {
				$this->public_title = 'Novalnet'.' '. MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_TEXT_PUBLIC_TITLE;
				$this->title = 'Novalnet'.' '.MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_TEXT_TITLE;
			}
            if ($this->enabled == 'true' && (!$this->vendor_id || !$this->auth_code || !$this->product_id || !$this->tariff_id || !$this->key )) {
                $this->title .= '<span class="alert">' . MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_NOT_CONFIGURED . '</span>';
            } elseif ($this->test_mode == '1') {
                $this->title .= '<span class="alert">' . MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_IN_TEST_MODE . '</span>';
            }
        }
    }
    ...
    function selection() {
        ...
        $selection = array('id' => $this->code,
            ...
            'fields' => array(array('title' => '', 'field' => MODULE_PAYMENT_NOVALNET_INFORMATION_PAYMENT_INSTANT),
    ...
    function pre_confirmation_check() {
        ...
        if (count($HTTP_POST_VARS) == 0 || $HTTP_POST_VARS == '')
            $HTTP_POST_VARS = $_POST;
        $error = '';
        if (!function_exists('curl_init') && ($this->code == 'novalnet_instantbanktransfer')) {
            ini_set('display_errors', 1);
            ini_set('error_reporting', E_ALL);
            $error = MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_CURL_MESSAGE;
        }
    ...
    function process_button() {
        ...
        $test_mode = $this->encode((strtolower(MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_TEST_MODE) == 'true' or MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_TEST_MODE == '1') ? 1 : 0);
        ...
        if (strstr($checkout_url, '?')) {
            ...
            if (substr($checkout_url, -1) == '?')
                $error_url = $checkout_url . 'payment_error=' . $this->code . '&error=$ERROR_MESSAGE';
            else
                $error_url = $checkout_url . '&payment_error=' . $this->code . '&error=$ERROR_MESSAGE';
        }
        else
    ...
    function before_process() {
        ...
        if (isset($_POST['status']) && $_POST['status'] == 100) {
            ...
            if ($_REQUEST['test_mode'] == 1) {
                $order->info['comments'] .= '<B><br>' . MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_TEST_ORDER_MESSAGE . '</B>';
            }
            $order->info['comments'] .= '<B><br>' . MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_TID_MESSAGE . $HTTP_POST_VARS['tid'] . '</B><br />';
    ...
    function after_process() {
        ...
        if ($_SESSION['nn_tid'] != '') {
            ...
            $urlparam .= '&auth_code=' . $this->auth_code . '&status=100&tid=' . $_SESSION['nn_tid'] . '&vwz2=' . MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_TEXT_ORDERNO . '' . $insert_id . '&vwz3=' . MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_TEXT_ORDERDATE . '' . date('Y-m-d H:i:s') . '&order_no=' . $insert_id;
    ...
    function install() {
        ...
        /* $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('".$allowed_title."','MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_ALLOWED', '','".$allowed_desc."', '6', '0', now())"); */

        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('" . $enable_title . "', 'MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_STATUS', 'True', '" . $enable_desc . "', '6', '0', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('" . $vendor_title . "', 'MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_VENDOR_ID', '', '" . $vendor_desc . "', '6', '2', now())");

        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('" . $auth_title . "', 'MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_AUTH_CODE', '', '" . $auth_desc . "', '6', '3', now())");

        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('" . $product_title . "','MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_PRODUCT_ID', '', '" . $product_desc . "', '6', '4', now())");

        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('" . $tariff_title . "', 'MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_TARIFF_ID', '', '" . $tariff_desc . "', '6', '5', now())");

        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('" . $password_title . "', 'MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_PASSWORD', '', '" . $password_desc . "', '6', '6', now())");

        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('" . $sortorder_title . "', 'MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_SORT_ORDER', '0', '" . $sortorder_desc . "', '6', '7', now())");

        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('" . $enduser_title . "', 'MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_INFO', '', '" . $enduser_desc . "', '6', '8', now())");
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('" . $proxy_title . "', 'MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_PROXY', '', '" . $proxy_desc . "', '6', '11', now())");
    ...
    function keys() {
        return array( 'MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_LOGO_STATUS','MODULE_PAYMENT_NOVALNET_INSTANTBANKTRANSFER_STATUS',
